...
function isStaticModeEnabled() {
  const raw = process.env.SHELLO_STATIC_MODE;
...
function writeRuntimeConfig() {
  ...
  const body =
    "// Generated by scripts/prepare-vercel-output.mjs — do not edit by hand.\n" +
    "// Override the SHELLO_STATIC_MODE env var on the deployment to toggle modes.\n" +
    "window.__SHELLO_CONFIG__ = window.__SHELLO_CONFIG__ || {};\n" +
    `window.__SHELLO_CONFIG__.staticMode = ${staticMode ? "true" : "false"};\n`;

  const targets = [
    path.join(deployOut, "shello-config.js"),
    path.join(deployOut, "ka", "shello-config.js"),
    path.join(deployOut, "en", "shello-config.js")
  ];
  ...
  console.log(`Wrote shello-config.js with staticMode=${staticMode}`);
}
